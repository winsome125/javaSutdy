...
/*
 ...
 - main() 메서드가 실행되기 전에 초기화 되므로 main()메서드 실행시 별도의 선어없이 변수를 사용할 수 있다.
 
 접근방법
 - 클래스내부
     : 변수명으로 접근할 수 있다.
       일반적인 멤버변수와 동일하게 접근한다. 
 - 믈래스외부
...
class MyStatic {
	/*
	 멤버변수 : 인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한 후 참조변수를 통해 접근해야 한다.
	 즉, 일반적인 멤버변수는 인스턴스를 생성하는 시점에 메모리에 로드된다.
	 (멤버변수는 Stack영역에, 인스턴스는 Heap영역에 생성된다.)
	 */
	int instanceVar;
	/*
	 정적멤버변수 : 객체를 생성할 피룡없이 클래스명으로 바로 접근할 수 있다.
	       JVM(자바가상머신)이 프로그램을 시작할때 이밈 메모리(Method영역)에 로드된다.
	 */
	static int staticVar;
	/*
	 인스턴스형 멤버메서드 : 인스턴스형 메서드에서느 모든 멤버를 사용할 수 있다.
	        인스턴스형 멤버, 정적멤버 둘 다 접근이 가능하다.
	 */
	void instanceMethod() {
	...
	/*
	 정적멤버메서드 : 정적메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다.
	 정적 멤버만 접근할 수 있다.
	 (※ main 메서드에서는 static으로 선언된 메서드만 호출할 수 있었음을 기억하자.)
	 */
	static void staticMethod() {
		// System.out.println("instanveVar="+ instanceVar); // 에러발생
		System.out.println("staticVar="+ staticVar);
		// instanceMethod(); //에러발생
	}
}
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 일반적인 멤버변수에 접근하는 방법
		  : MyStatic클래스의 인스턴스를 생성한 후 참조변수를 통해 접금한다.
		 */
		...
		/*
		 static(정적) 멤버 변수 접근방법
		  : 클래스 명을 통해 직접 접근한다. 인스턴스를 생성하지 않고 즉시 접근할 수 있다.
		 */
		...
		/*
		 ...
		 (※ 참조변ㅅ를 통해 접근할거면, 굳이 정적변수를 사용할 이유가 없기 때문이다.)
		 */
		MyStatic.staticVar = 300;
		System.out.println("myStatic.staricVar="+ myStatic.staticVar);
		...
		/*
		 인스턴스형 변수는 각각의 인스턴스 내부에 존재하므로 아래 2개의 값은 서로 다르게 출력된다.
		 */
		ms1.instanceVar = 100;
		...
		System.out.println("ms2.ststicVar="+ ms2.staticVar); //900
		/*
		...
		/*
		 ...
		 인스턴스메서드는 인스턴스 참조면수를 통해서만 접근할 수 있다.
		 */
