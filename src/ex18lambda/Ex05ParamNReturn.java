 ...
 */
@FunctionalInterface
interface RandomNunmber{
	public int randomNum();
}

/*
 매개변수로 반반환값 둘 다 있음
  : 매개변수로 전잘된 두 정수마늨므이 구구잔을 2차원배열로 반환한다.
 */
@FunctionalInterface
...
public class Ex05ParamNReturn {
	...
	public static void main(String[] args) {
		...
		String s2 = "람다식입니다.";
		/*
		 2개의 매개변수는 String으로 추상메서드를 통해 유추할 수 있으므로 람다식에서는 생략할 수 있다.
		 메서드명을 제거한 후 괄호사이에 화살표를 삽입하여 람다식을 정의한다.
		 호출시에는 부모인터페이스의 추상메서드인 makeString()을 이용하면 된다.
		 */
		StringConcat sc = (a, b)-> {
		...
		System.out.println("\n### 람다식2");
		/*
		 매개변수가 없는 람다식을 정의해야 하므로 반 소과롷를 사용한다.
		 난수를 생성ㅎ해서 반화하는 기능을 정의하였다.
		 */
		RandomNunmber randomNunmber = () -> {
			Random random = new Random();
			return random.nextInt(100);
		};
		// 반환값이 있으므로 좌측
		int rNumber = randomNunmber.randomNum();
