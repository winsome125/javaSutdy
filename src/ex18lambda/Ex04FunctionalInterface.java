...
/*
 함수형 인터페이스
  : 람다식을 사용하기 위해서는 인터페이스를 먼저 만들고 람다식으로 구현현 할 추상메서드를 선언한다.
...
/*
 @FunctionalInterface
  : 함수형 인터페이스의 조건을 갖추었는지에 대한 검사를 컴파일러에게 요청하는 역할을 한다.
 */
@FunctionalInterface
interface ISchool4{
...
public class Ex04FunctionalInterface {
	...
	/*
	 ...
	 아래와 같이 매개변수로 전달받은 후 추상메서드인 studyLambda()를 초훌할 수 있다.
	 */
	static void execute(ISchool4 school4, String s) {
		/*
		 school4 => main에서 선언한 라다식을 받은 매개변수
		 s => 전달된 문자열
		 인터페이스에 정의된 차상메서드 : studyLambda(String str)
		 따라서 아래와 같이 호출할 수 있다.
		 */
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 추상메서드를 통해 매개변수의 타입을 유추할 수 있으므로 아래와 같이 String을 생략할 수 있다.
		 */
