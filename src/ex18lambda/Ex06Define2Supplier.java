...
public class Ex06Define2Supplier {
	...
	public static void main(String[] args) {
		...
		/*
		 Supplier<T>
		  : 공급자라는 의미를 가지고있다.
		    매개변수는 없으나 리턴값은 있는 get() 메서드가 정의되어있다.
		    주로 데이터를 생성하여 리턴한다.
		    
		  interface Supplier<T> {
		     T get();
		  }
		  여기서 타임매개변수 T는 get() 메서드의 반환타입으로 정의된다.
		 */
		...
		// 추상메서드만 get()을 통해 람다식을 호출한다.
		int rNum = sup.get();
		System.out.println("생성된난수="+ rNum);
		
		// 난수를 생성하는 람다식 sup와 정수 10을 인수로 전달한다.
	...
	/*
	 ...
	 이때 cnt만큼 반복하게되므로 결국 난수 10개릎 생성하여 List컬렉션에 저자한 후 반환하고있다.
	 */
	public static List<Integer> makeRandomNum(Supplier<Integer> s, int cnt){
		List<Integer> li = new ArrayList<Integer>();
		for(int i=1 ; i<=cnt ; i++) {
			/*
			 랃다식의 get()메서드 호출을 통해 난수를 생성한다.
