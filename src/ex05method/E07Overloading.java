...
/*
 메서드 오버로딩(Method Overloading)
  ...
  - 반환타입만 다른것을 허용되지 않는다.
  - 컴파일러는 메서드 호출시 전달되는 인수(피라미터)를 통해 호출할 메서드를 구분한다.
 */
public class E07Overloading {
	
	//주문번호와 군번을 매개변수로 정의한 매서드(남자를 표현)
	...
	/*
	 해당 메서드는 아펭서 선언과 반한타입만 다른 경우이므로 person(10)으로 호출한다 가정했을 때 어떤 메서드를 호출할지 애매하다.
	 컴파일러는 애매한 경우 에러를 발생시킨다.
	 따라서 아래의 메서드느 오버로딩이 성립하지 않는다.
	 */
	// static void person(int juminNum) {
