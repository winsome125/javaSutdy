...
public class Teacher extends Person {
	...
	/*
	 시나리오]
	 Teacher객체는 나이와 과목이 같으면 동일한 객체로 처리하도록
	...
	/*
	 hashCode()
	  : 객체가 가진 고유한 주소값을 정수형으로 반환해준다.
	    ...
	    정수형 멤버변수는 그냥 사용하면되고, 
	    객체형 멤버변수는 hashCode() 메서드를 통해 정수값으로 반환되는 참조값을 사용하면된다.
	 */
	@Override
	public int hashCode() {
		...
		/*
		 방법1
		  : 정수형변수와 객체의 해시코득밧을 함산한 후 그대로 반환한다.
		 */
		...
		/*
		 방법2
		  : 방법1이 번거롭ㄷ면 Objects 클래스의 정적메서드인 hash()를 사용해도된다.
		    멤버변수의 갯수만큼 인수로 전달하면 위와 동일하게 고유한 주소값을 정수형으로 반환해준다.
		 */
	...
	/*
	 equals()
	   : 객체가 가진 멤버변수의 값을 비교한다
	     값이 같을때는 true 다를때는 false를 반화하도록 오버라이딩한다.
	 */
	@Override
	public boolean equals(Object obj) {
		...
		if ( (teacher.getAge()==super.getAge()) && teacher.subject.equals(this.subject)) {
			/*
			 모든 내용이 일치하면 true를 반환한다.
			 그러면 set컬렉션에는 add(추가)되지 않는다.
			 */
			return true;
		}
		else {
