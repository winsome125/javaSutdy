...
/*
 for문
  : while문과 같은 반복문으로 초기값, 조건식, 증감식이 한줄에 있어 반복의 횟수를 명확히 알수있는 반복문이다.
...
public class E05For {
	...
	public static void main(String[] args) {
		...
		/*
		 시나리오] for문을 이용하여 1~10까지의 정수중 2의배수의 합을 구하는 프로그램을 작성하시오
		 */
		...
		System.out.println("1~10사이의 2의배수 합(방법1):"+ total);
		
		/*
		 방법2 : if문 없이 초기값을 0부터 시작하여 2씩 증가한다.
		 */
		total = 0;
		for(int i=0 ; i<=10 ; i+=2) {
			total += i;
		}
		System.out.println("1~10사이 2의배수희합(방법2):"+ total);
		
		// for문으로 무한루프를 만뜰때는 (;;)와 같이 표현한다.
		int a =1;
		for(;;) {
		...
		/*
		 for문의 끝에 ;을 사용하던 반복할 문장이 없는것으로 간주되어 for문과 상관없이 한번만 실행된다.
		 */
		for(int j=0 ; j<=5 ; j++);
		{
			System.out.println("어랏... 나는 누구? 여긴 어디? 졸려..");
		}
		
		/*
		 for문의 초기식에서 선언된 변수 j는 for문이 종료되는 순간 메모리에서 소멸된다.ㅓ
		 여기서 사용된 변수 j는 지역변수 (Local variable)이라고 한다.
		 지역변수는 해당 지역을 벗어나면 즉시 소멸되는 특징을 가지고있다.
		 */
		// j는 for문이 종료된때 소멸되므로 아래문장은 에러발생
		// System.out.println("위 for문에서 선언한 변수 j="+ j);
		System.out.println("위 main함수 지역에서 선언한 변수 total="+ total);
		
		/*
		 for문의 초기값을 외부(넓은지역, 여기서는 main지역)에서 선언하면 해당변수는 for문의 안쪽(좁은지역)에서 사용할 수 있다.
		 즉 아래에 선언한 변수 i는 main메서드의 지역변수로 선원된다.
		 */
		int i=0;
		for( ; i<=5 ; i++) {
