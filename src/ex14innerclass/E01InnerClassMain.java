...
/*
 ...
 - 외부 클래스의 모든 멤버(정적 혹은 인스턴흐형)를 사용할 수 있다.
...
class Unit{
	...
	public Unit(String name, int healthPoint) {
		...
		// 인스턴스형 멤버변수 객체생성
		marine = new Marine();
		// Medic인스턴스 생성은 주석으로 처리하고 외부에서 생성된다.
		// medic = new Medic();
	}
...
public class E01InnerClassMain {
	...
	public static void main(String[] args) {
		...
		/*
		 외부클래스가 아닌 다른 클래스에서는 내부 멤버 믈래스가 보이지 않으므로 직접 인스턴스를 생성할 수 없다.
		 */
		//Marine m = new Marine(); //에러발생
		
		/*
		 외부클래스의 인스턴스를 먼저 생성한 후 내부클래스의 인스턴스를 생성해야한다
		 Marine은 Unit클래스의 생성자에서 이미 인스터스 생성이 되어있으므로 아래와 같이 사용할 수 있다.
		 */
		Unit.Marine marine = new Unit("마린", 50).marine;
		marine.attack();
		
		/*
		 Medic의 경우 Unit의 생성자에서 인스턴스 생성이 되지 않았으므로 아래와 같이 new를 통해 생성해야한다.
		 이때에도 Unit.인스턴스를 통해 생성해야하므로 new를 두번 사용해서 인스턴스를 생성한다.
		 */
