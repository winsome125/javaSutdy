...
/*
 ...
 - 이름이 없는 클래스를 말한다.
 - 내부클래스처럼 이벤트 기반의 프로그램잉에서 많이 사용된다.(안드로이드)
...
class Singer {
	...
	/*
	 Object클래스에 정의된 메서드로 객체가 가진 값을 문자열로 반환해주는 메서드이다.
	 java에서 정의하는 모든 클래스는 Object클래스를 상속하므로 이와같이 오버라이딩이 가능하다.
	 */
	@Override
...
class GirlGroup extends Singer {
	...
	// 부모의 toString()을 호출하기 위헤 super를 사용한다.
	@Override
	public String toString() {
		// 이름과 팀명을 반환한다.
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		 GrilGroup클래스를 상속받은 익명클래스 정의
		 : Singer타입의 변수에 GrilGroup을 상속받은 익명클래스를 생성해서 그 주소값을 할당한다.
		   개념적으로 new GrilGroup(){} => 익명 extends GirlGroup 과 같다.
		   이름이 없는 인스턴스 이므로 부모클래스의 일므을 빌려서 자식의 영역을 만든다고 생각하면 된다.
		 */
		...
		/*
		 부모의 참조변수로 자식을 참조할 수 있지만, 접근범위가 부모까지 제한된다.
		 따라서 아래와 같이 자식에서 확장한 멤버변수는 접근할 수 없어 에러가 발생한다.
		 접근하고 싶다면 다운캐스팅(강제형변환)을 해야지만 익명클래스는 이름이 없기 때문에 다운캐스팅 자체가 불가능하다.
		 따라서 익명클래스에서느 멤버의 확장은 거의 사용되지 않는다.
		 대부분 오버라딩을 목적으로 활용된다.
		 */
