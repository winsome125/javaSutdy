...
public class E05Date {
	...
	/*
	 Calendar클래스를 통해 얻은 인스턴스를 초로 변환한 후 매개변수로 전달된 날짜서식으로 변경한 String를 반환한다.
	 */
	public static String myDateFormat(Calendar c, String formatStr) {
		/*
		 getTime()
		  : 서버의 표준서식인 1970-01-01 부터 현재까지의 시간을 초 단위로 반환해준다
		    이를 타임스템프라고 한다.
		 */
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		...
		/*
		 Calendar 클래스는 대표적인 싱글톤 객체이므로 new로 인스턴스를 생성할 수 없고, 유틸리티 메서드를 통해 참조값을 얻어온다.
		 */
		Calendar calendar = Calendar.getInstance();
		
		/*
		월을 표현하는 Month, 분을  표현하는 Minute의 이니셜이 동일하기 때문에 구분을 위해 월은 대문다, 분은 소문자를 사용한다. 
		 */
		...
		/*
		 현재 날짜를 기준으로 전/후를 구하기 우해 add()메서드를 사용한다.
		 마약 음수를 주면 과거의 날짜를 구할 수 있다.
		 */
		...
		/*
		 ...
		 여기서는 두가지 서식의 날짜를 파싱하여 우한 날짜객체를 통해 계산한다.
		 */
		// 오늘날짜 : 0000-00-00 형식
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			/*
			 getTime()으로 얻어오는 시간은 13자리의 정수값인데 10자리는 순수한 초(Sec)이고, 11~13자리는 밀리세컨즈를 의미한다.
			 즉 100으로 나눠서 뒤의 3자리를 제거하면 순수한 초가 반환된다.
			 또한 하루는 60*60*24 즉 86400초 이므로 아래와 같이 남은 날짜를 구할 수 있다.
			 */
