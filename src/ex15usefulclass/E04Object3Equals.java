...
class MyClass{
	...
	인스턴스가 가진 멤버변수의 값에 대한 비교를 통해 동일여부를 판단한다.
	equals()메서드의 매개변수 타입은 모든 인스턴스를 대항하므로 Object형으로 정의되어 있다. 
	 */
	@Override
	public boolean equals(Object obj) {
		/*
		 매개변수로 전달된 인스턴스를 Object로 받으면 업캐스팅이 되므로 다운캐스팅 이후에 비교를 해야된다.
		 이때 전달된 인스턴스가 MyClass형이 아니라면 비교의 대상이 될 수 없으므로 상속관계 확인을 위해 instanceof연산자를 사용한다.
		 */
		if(obj instanceof MyClass) {
			/*
			 매개변수가 MyClass 타입이 맞다면 비교를 위해 다운캐스팅한다..
			 그렇지 않으면 부모타입으로는 자식 맴버에 접근할 수 없으므로 멤버변수 data에 대한 비교가 불가능해진다.
			 */
			...
			/*
			 매개변수로 전달된 인스턴스가 MyClas 타입이 아니라면 비교의 대상이 될 수 없으므로 false를 반환한다.
			 */
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		/*
		 마약 오버라이딩 해놓은 equals()를 사제한 후 비교하면 다르다는 결과가 나오게된다.
		...
		/*
		 print()롤 통해 출력하면 자동으로 toString()을 호출하므로 아래 출력결과는 동일하다.
		 */
