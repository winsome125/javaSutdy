...
public class InsertSQL extends IConnectImpl {
	...
	public void execute() {
		try {
			/*
			 1.쿼리문준비 
			    : 값의 세팅을 위한 부분을 ?(인파라미터)로 기술한다.
			 */
			String query = "INSERT INTO member VALUES (?, ?, ?, ?)";
			
			// 2.prepared개게생성 : 준비한 쿼리문을 인수로 전달한다.
			...
			/*
			 현재날짜를 Java에서 입력하는 경우 아래와 같이 변환가정을 거쳐야한다.
			 util패키지로 시간을 얻어온 후 sql패키지로 타입을 변환한다.
			 이때는 date형으로 입력되므로 setDate()로 인파리터를 설정해야한다.
			 */
