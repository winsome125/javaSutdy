...
public class FunctionCall extends IConnectImpl {
	...
	public void execute() {
		try {
			...
			/*
			 첫번째 피라미터는 함수 실행후 반환값에 대한 타입을 지정한다.
			 Types 클래스의 정적상수를 사용한다. 
			 */
			csmt.registerOutParameter(1, Types.VARCHAR);
			/*
			 두번째 피라미터는 사용자로부터 입력받은 아이디(문자열)에 대한 설정을 한다.
			 여기서 입력받은 값은 오라클로 전달된ㄷ. 
			 */
			csmt.setString(2, scanValue("아이디"));
			// 여기서 함수를 실행한다.
			csmt.execute();
			// 실행 후 반환값은 첫번째 피라미터로 입력되므로 getXX(1)로 출력한다.
