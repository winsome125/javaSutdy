...
public class SelectQuery extends ConnectDB {
   ...
   /*
    ResultSet 인터페이스
     : select문 실행시 쿼리의 실행결과를 ResultSet객체를 통해 받는다.
       결과로 반환된 레코드의 처음부터 긑까지 next()메서드를 통해 확인 후 반복 인출한다.
       
       컬럼의 자료형이
         number타입 : getInt()
         char/varchar2개타입 : getString()
         date타입 : getDate() 메서드를 통해 출력한다.
       해당 메서드의 이수는 인덱서(1부터시작) 혹은 컬럼명(or 별칭)을 사용할 수 있다.
   ...
   public void execute() {
      try {
         ...
         /*
          - executeUpdate()
             : insert, update, delete와 같이 기존 레코드에 변화가 생기는 경우에 사용한다.
               실행 후 적용된 행의 갯수가 int형으로 반환된다.
              
          - executeQuery()
             : select 쿼리문을 실행할때 사용하는 메서드로 레코드에 영향을 미치지않고,
               단순히 조회만 하는 경우 사용한다.
               조회한 레코드를 반환값으로 받아야하므로 ResultSet객체가 반환타입으로 정의되어있다.
          */
         ...
         while(rs.next()) {
            ...
            /*
             ...
             또한 자료형이 Date()이므로 java.sql패키지의 클래스를 사용해야한다.
             */
            java.sql.Date regidate = rs.getDate("regidate");
            
            /*
             날짜를 getString()으로 인출하면 시간까지 포함되서 출력된다.
             0000-00-00 00:00:00 형식이므로 적당한 크기로 자르고 싶다면 substring() 메서드를 사용하면된다.
             */
