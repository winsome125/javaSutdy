...
/*
 ...
 2. 생성자에서 예외발생시 출력할 메세지를 super()를 입력한다.
 3. 예외 발생지점에서 if문으로 감지한 후 예외 객체를 생성 및 throw한다.
...
public class Ex06DeveloperDefine {
	...
	public static int readAge() throws AgeErrorException{
		...
		try {
			/*
			 나이 입력시 문자를 잘 못 입력하는 경우에 대해 예외처리를 하고있다.
			 이 부분은 Java에서 미리 정의해놓은 예외처리 클래스를 사용한다.
			 */
			inputAge = sc.nextInt();
		}
		...
		/*
		 ...
		 개발자가 정의한 예외객체를 생성한 후 직접 throw 한다.
		 */
		if(inputAge<0) {
			...
			/*
			 ...
			 이를 반드시 catch 해야되고,
			 그렇지 않으면 JVM에 의해 예외객체가 throw하는것과 동일하게 프로그램이 비정상 종료된다.
			 */
			throw ex;
		}
		return inputAge;
