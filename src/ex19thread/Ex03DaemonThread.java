...
/*
 쓰레드로 구현하지 않은 일반적인 클래스
  : 하나의 클래스가 작어블 완료해야 다음 클래스의 작업을 진행할수있다.
    즉 동시에 작업을 진행할 수 없다.
 */
class NormalClass{
...
/*
 독립쓰레드(Non Daemon Thread)
  : 메인쓰레드와 Working쓰레드(개발자가 정의한 쓰레드)를 일컫은 것으로 메인쓰레드가 종료되어도 계속 실행되며,
    Dead상태가 되어야만 실행이 종료되는 쓰레드를 말한다.
    해당 쓰레드는 main() 메서드가 종료된 이후에도 계속 실행되는것을 볼수있다.
 */
class NormalThread extends Thread{
	/*
	 기본생성자를 통해 인스턴스를 생성하면 쓰레드명이 JVM에 의해 자동 부여된다.
	 */
	public NormalThread() {}
	/*
	 Thread클래스의 생성자 호출시 전달하는 이수를 통해 해당 쓰레드의 이름을 지정할 수 있다.
	 이때 지정된 이름은 getName()을 통해 반홤받을 수 있다.
	 */
	...
	/*
	 쓰레드 동작중 비정상적으로 종료될때 InterruptedException이 발생할 수  있으므로, 예외처리를 해야한다.
	 여기서는 즉시 처리하지않고, throws해서 호출한 위치로 예외객체를 던지기한다.
	 */
	void threadMethod() throws InterruptedException{
		/*
		 10번 반복중 2초씩 block 상태로 들어가므로 총 20초간 반복된다.
		 반복횟수와 쓰레드명을 출력한다.
		 */
		for(int i=1 ; i<=10 ; i++) {
...
/*
 종속쓰레드(Daemon Thread)
  : 모든 독립쓰레드가 종료되면 자동으로 종료되는 쓰레드를 말한다.
    메인메서드도 하나의 독립쓰레드이므로, 다른 독립쓰레다가 남아있다면 종료될때까지는 프로그램이 종료된것이 아니다.
	...
	public void run() {
		while(true) {
			...
			try {
				...
				System.out.println("자동저장시 오류발생ㅜㅠ");
			}
		}
	}
}
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/*
		 쓰레드로 구현하지 않는 일반적인 클래스의 작업으로 순차적으로 진행되어 1번 인스턴스가 작업을 마쳐야만 2번째가 실행된다.
		 */
		NormalClass nt1 = new NormalClass("첫번재 클래스");
		nt1.classmethod();
		...
		/*
		 데몬쓰레드는 독립쓰레드르 먼저 생성한 후 setDaemon()을 통해 데몬쓰레드로 만들어준다.
		 */
		...
		/*
		 Thread클래슫의 정적메서드를 통해 쓰레드의 여러상태를 확인할 수 있다.
		 */
