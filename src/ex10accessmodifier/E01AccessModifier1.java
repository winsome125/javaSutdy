...
/* class를 정의할때는 반드시 public 이나 default(생략형)만 사용할 수 있다.
...
/* public 클래스는 해당 Java파일을 대표하는 클래스로 파일명과 동일해야한다.
   ...
   public으로 정의하는 경우 다른 패키지에서도 접근을 허용하게 되므로 하나의 Java파일에 하나의 class만 정의하는것을 권장한다. */
public class E01AccessModifier1 {
   ...
   private void privateMethod() {
	   /*
	    동일 클래스 내에서는 private 맴버라 할지라도 접근을 허용한다.
	    즉 동일 클래스 내에서는 접근지정자의 영향을 받지않는다.
	    */
      privateVar = 100; // 사용가능
   ...
   /*
    private으로 선언된 메서드도 클래스 내부에서는 호추로할 쇼ㅜ 있다.
    맴버변수와 동일한 접근범위를 가진다.
    */
