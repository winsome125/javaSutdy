...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		// Map 컬렉션 생성. Key와 Value를 모두 String으로 선언
		...
		/*
		 중복저장
		  : 기존에 입력된 Key값 name이 이미 있으므로 '홍길동'이 반환된다.
		    기존에 값은 '최길동'으로 수정된다.
		 */
		System.out.println("name이라느 키값으로 저장된 이전의 값:"+ map.put("name", "최길동")); //덮어쓰기
		// 중복 저장되므로 4개가 출력된다.
		...
		/*
		 2.키값을 모를 때 (혹은 키값이 너무 많아서 하나씩 명시하기 힘들때)
		  : KeySt()을 통해 전체 Key를 Set컬렉션으로 얻어온다.
		    해당 반환값을 확장 for문을 통해 반복하고,
		    이를 통해 Value를 추출할 수 있다.
		 */
		...
		/*
		 Value만 얻어올때는 value() 메서드를 통해 값을 추출할 수 있다
		...
		System.out.println(map.containsValue("남자") ? "여자 있다" : "여자 없다");
		
		/*
		 객체 삭제
		  : key를 통해 삭제하고, 삭제에 성하면 해당 value가 반환된다.
		 */
