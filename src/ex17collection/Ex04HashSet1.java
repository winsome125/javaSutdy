...
/*
 ...
 - 하지만 개발자가 정의한 클래스라면 hashCode(), equals()메서드를 적절히 오버라이딩해야만 중복을 제거할 수 있다.
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		/* Object를 가빈으로 set컬렉션을 생성했으므로 모든 객체를 저장할 수 있다. */
		HashSet<Object> set = new HashSet<Object>();
		
		/* 3가지의 기본클래스와 우리가 정의하 Teacher클래스의 인스턴스를 생성한다. */
		String varString1 = "JAVA";
		String varString2 = new String("개발자");
		Date varDate= new Date();
		int varInt = 100; // 저장시 Integer로 Boxing처리됨
		Teacher varTacher = new Teacher("김봉두", 55, "체육");
		
		/*
		 객체저장
		  : add()메서드를 통해 저장하고, 성공하면 true를 반환한다.
		 */
		System.out.println("add결과:"+set.add(varString1));
		set.add(varString2);
		set.add(varDate);
		set.add(varInt); //new Integer(varInt)와 같이 Boxing처리됨
		set.add(varTacher);
		...
		while(itr.hasNext()) {
			...
			/*
			 HashSet<E>에 저장시 모두 Object형으로 자동형변횐되어 저장되므로
			 우리가 직접 정의한 클래스에 대해서만 instanceof를 통해 타입을 확인 후 다운캐스팅 하여 출력한다.
		...
		/*
		 객체삭제
		  : set컬렉션은 인덱스가  없으므로 객체의 참조값을 통해서만 삭제할 수 있다.
		 */
