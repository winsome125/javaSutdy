...
//		list.add(5,"소방차ㅋㅋ"); // 에러 발생됨
		...
		/*
		 중복저장 
		 : List는 배열의 특성을 가지므로 데이터를 중복저장할 수 있다.
		   동일한 데이터라도 index로 구분할 수 있기 때문이다.
		   ad()는 추가에 성공할 경우 true를 반환한다.
		 */
		...
		/*
		 ...
		 방법1 : 일반 for문 사용.
		        이때는 이넥스를 통해 접근해야 하므로 get()메서드를 사용한다.
		 */
		System.out.println("\n[일반for문 사용]");
		...
		/*
		 방법2
		 : 확장 for문의 경우 인덱스 없이 전체 원소를 반복해 주므로 좀 더 간단히 표현이 가능하다.
		   따라서 가자 많이 사용된다.
		 */
		System.out.println("\n[확장for문 사용]");
		for(String obj : list) {
		...
		/*
		 방법3 : Iterator사용법
		   ...
		   2. hasNext()로 출력한 인스턴스가 있는지 검사한다.
		...
		/*
		 ...
		 방법1 : 인덱스로 삭제
		   remove(인덱스) : 인덱스를 통해 삭제를 진행한 후 삭제가 완료되면 해당 인스턴스를 반환한다.
		                  ...
		                  삭제 성공시 해당 인스턴스를 반환하므로 삭제 후 즉시 확인할 수 있다.
		 */
		Object obj = list.remove(2);
		// 2번 인덱스 삭제 => 트와이스를 반환한다.
		System.out.println("삭제된객체:"+ obj);
		
		//indexOf()를 통해 해당 인스턴스릐 인덱스를 찾은 후 삭제한다.
