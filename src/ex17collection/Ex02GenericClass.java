...
class FruiBox{
 ...
 => 기존 Object기반의 FruitBox를 아래와 같이 제네릭 클래스로 변경한다.
    자료형에 해당하는 부분을 타입매개 변수로 교체하고 인스턴스 생성시 자료형을 결정하기 위해 클래스명 <T>형태로 변경한다.
*/
//제네릭 클래스 정의 
class GenericFruitBox<T>{
	T item;
	public void stre(T item) {
		this.item = item;
	}
	public T pullOut() {
		return item;
