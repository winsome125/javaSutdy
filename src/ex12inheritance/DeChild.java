...
public class DeChild extends DeParent {
	...
	public void study() {
		/*
		 부모의 멤버변수 name은 private이므로 자식에서 사용하기 위해서는 getter를 통해 접근해야한다.
		 */
	...
	/*
	 오버라이딩 아님 
	 : 부모클래스에서 private켐버로 선언 하였으므로 자식쪽에서는 보이지 않아 오버라이딩의 대상이 될 수 없다.
	...
	/*
	 메서드 오버라이딩의 조건
	 - 부모클래스에서 선언될때오 ㅏ비교해서 접근범위가 통일하거나 그보다 넓으면 가능하다.
	 - 부모에서 default로 선언하면 자식에서는 default 혹은 그보다 넓은 public으로 선언하면 오버라이딩이 가능하다.
	 */
	@Override
	public String sleep() {
	...
	/*
	 ...
	 즉 이 경우느 오버로딩으로 자식쪽에서 확장한 메서드가 된다.
	...
	/* 
	 ...
	 클래스 외부(메서드 영역)에 미리 로드되므로 오버라이딩 되지않고, 호출할 때 각 클래스명을 통해 사용해야한다.
	 */
