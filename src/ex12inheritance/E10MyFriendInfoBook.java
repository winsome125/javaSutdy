...
class HighFriend extends Friend {
   ...
   /*
   오버라이딩1
   : 고딩친구의 전체정보를 출력하기 위해 부모에서 정의한 메서드를 super를 통해 호출하고,
   ...
   */
   ...
   /*
   오버라이딩2
   : 자식클래스에서 오버라이딩하여 재정의한 메서드로 고딩친구의 간략한 정보를 출력한다.
...
public class E10MyFriendInfoBook {
   ...
   */
   ...
   /*
    ...
    따라서 선택한 메누에 따라 핸들러 클래스의 메스드만 호출하는 형태로 구현되어있다.
    */
...
/*
해당 프로그램에서 기능을 담당하는 클래스로 핸들러 혹은 메니져클래스라고 부른다.
*/
class FriendInfoHandler {
   // 멤버변수
   /*
   상속관계에서 부모클래스로 인스턴스 배열을 생성한다.
   부모로 자식을 참조할 수 있으므로, 부모타입의 배열에는 자식타입의 인스턴스를 저장할 수 있다.
   */
...
//      System.out.println("addFriend 호출됨");
      ...
      if(choice==1) {
         ...
         /*
         참조변수를 인스턴스 배열에 추가한다.
         인덱스로 사용된 변수의 초기값이 0이므로, 0번 인덱스에 추가한 뒤 1 증가시킨다.
         이를 위해 '후위증가' 하고 있다.
         */
      ...
      /*
      더이상 실행할 문장이 없으므로 해당 메서드는 메모리에서 소멸되고,
   ...
   /*
    ...
    3. 정보 출력시 배열에 입력된 캐게수만큼 반복하여 각 원소를 통해 정보를 출력한다.
    4. 출력을 위한 멤버메서드는 상속관계에서 오버라이딩 처리되어 있으므로 참조변수의 영향을 받지 않고 항상 자식클래스에 오버라이딩 된 메서드가 호출된다.
    5. 즉, 저장된 인스턴스는 Friend타입이지만 오버라이딩을 통해 별도의 형변환이 필요하디 않다.
       항상 원하는 정보를 간단히 출력할 수 있게된다.
    */
   // 저장된 연락처 전체정보 출력
   public void showAllData() {
...
//      System.out.println("showSimpleData 호출됨");
      /*
       ...
       자식클래스에서느 오버라이딩을할 수 없으므로 개별적인 멤버메서드를 정의해야한다.
       정보가 저장되는 배열은 Friend타입으로 자식인스턴스를 저장하면 자동으로 형변환(업캐스팅)이 된다.
       이때 자식의 정보를 출력하는 메서드를 바로 호출할 수 없으르로 아래와 같이 일일이 확인한 후 강제형변환(댜운캐스팅) 해야단다.
	   for(int i=0; i<numOfFriends; i++) {
   ...
   public void searchInfo() {
	   ...
	   for(int i=0 ; i<numOfFriends ; i++) {
		   /*
		    ...
		    검색을 위해 입력한 이름과 비교해서 일치하는 경우에만 정보를출력한다.
   ...
   public void deleteInfo() {
	   ...
	   /*
	    배열의 원소 중 삭제된 원소의 이덱스값을 저자하기 위한 변수로 삭제한 후 빈자리를 채워넣을때 사용하나.
	    인덱스는 0부터 시작이므로 -1을 초기값으로 설정한다.
	    */
	   int deleteIndex = -1;
	   ...
	   for(int i=0 ; i<numOfFriends ; i++) {
		   ...
		   if(deletename.compareTo(myFriends[i].name)==0) {
			   /*
			    인스턴스 배열에서 샂게는 null로 변경하면된다.
			    참조값이 null이라는것은 참조할 인스턴스가 없다는 의미이므로 삭제한것과 동일한 결과가된다.
			    */
			   myFriends[i] = null;
			   // 삭제된 원소의 index를 저장한다.
			   deleteIndex = i;
	   ...
	   if(deleteIndex==-1) {
		   ...
		   /*
		    ...
		    만약 이 부분이 처리되지 않는다면 차후 검색이나 삭제시 NullPointerException이 발생할 수 있다.
		    */
		   for(int i=deleteIndex ; i<numOfFriends ; i++) {
			   myFriends[1] = myFriends[i+1];
		   }
		   // 삭제된 결과를 출력한다.
