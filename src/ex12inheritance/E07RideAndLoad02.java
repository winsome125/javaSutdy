...
public class E07RideAndLoad02 {
	...
	public static void main(String[] args) {
		...
		// 자식 인스터스를 자식 참조변수로 참조한다. (동일한 타입)
		DeChild child = new DeChild("율곡이이", 49, "00학번");
		
		/*
		 오버라이딩으로 처리된 메서드이므로 참조변수와 상관없이 항상 자식쪽 메서드가 호출된다.
		 메서드 오버라이딩은 자식쪼에 정의한 메서드가 부모쪽에 정의된 메서드를 가리게된다.
		 따라서 오버라이딩된 메서드르 호출할때는 참조변수의 영향을 받지않는다.
		 */
		child.excecise();
		child.sleep();
		child.PrintParent();
		...
		/*
		 부모타이의 참조변수를 통해 자식 인스터스를 참조할 수 있다.
		 상속관계이므로 가능하다.
		 */
		...
		/*
		 ...
		 자식객체가 생성될때 부모쪽의 메서드가 가려지게 되므로 참조변수에 영향을 전혀 받지 않는다.
		 */
		parent.excecise();
		parent.sleep();
		parent.PrintParent();
		...
		/*
		 부모클래스의 참조변수로 자식인스턴스를 참조하면 부모의 영역까지만 접근할 수 있으므로 자식영역에 정의된 정의된 메서드는 호출할 수 없다.
		 */
		// parent.study(); // 에러발생
		parent.walk();
		// parent.walk(20); // 에러발생
		
		// 정적메서드는 오버라이딩 되지 앟는다. 클래스명으로 호출한다.
