...
/*
 다형성(Polymorphism)
  : 하나의 인스턴스(객체)를 여러 타입의 참조변수를 통해 참조할때 참조변수의 타입에 따라 다양한 형태의 인스턴스를 이용할 수 있는 특성을 말한다.
...
class Child extends Parent{
	...
	void parentMethod(int childMember) {
		...
		System.out.println("Overriding:자식에서 확장한 메소드" + ":parentMethod(int childMember)");
	}
}
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		/*
		 자식타입으로 형변화 이후에 해당 참조변수를 통해 자식 멤버에 접근한다.
		 위와 동일한 방법이다.
		 */
		...
		/*
		 Java에서생성한 모든 클래스는 Object클래스를 상송한다.
		 따라서 모든 인스턴스는Object의 참조변수로 참조가 가능하다.
		 또한 Object클래스에 정의된 모든 메서드를 별도의 선언없이 사용할 수 있고,
		 필요하다면 오버라이딩도 할 수 있다.
		 */
